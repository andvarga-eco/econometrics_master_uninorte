...
model_flex=('lwage~C(P3271)+exp+C(educ)+C(sector)+C(occ)+(exp+exp2+exp3+exp4)*(C(educ)+C(sector)+C(occ))')
flex = smf.ols(model_flex, data=train)
results_flex = flex.fit()
print(results_flex.summary())
...
# calculating out-of-sample MSE
MSE_test_flex = sum((y_test - yhat_reg_flex)**2) / y_test.shape[0]
R2_test_flex = 1. - MSE_test_flex / np.var(y_test)

print("Test MSE for the flex model: " + str(MSE_test_flex))
print("Test R2 for the flex model: " + str(R2_test_flex))


# Modelo extra-flexible

model_extra=('lwage~C(P3271)+(exp+exp2+exp3+exp4+C(educ)+C(sector)+C(occ))**2')
